...
class WorkoutViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun getMuscleRecoveryFraction(): Map<MuscleGroup, Float> {
        val now = System.currentTimeMillis()
        val recoveryMap = mutableMapOf<MuscleGroup, Float>()
        
        MuscleGroup.values().forEach { recoveryMap[it] = 1f }
        
        val maxRecoveryMs = 48L * 60 * 60 * 1000L

        _workoutHistory.value.forEach { entry ->
            val timeDiff = now - entry.timestamp
            if (timeDiff < maxRecoveryMs) {
                val fraction = timeDiff.toFloat() / maxRecoveryMs.toFloat()
                entry.exercises.forEach { exSession ->
                    val def = _exerciseLibrary.value.find { it.name == exSession.name }
                    def?.muscleGroups?.forEach { mg ->
                        val current = recoveryMap[mg] ?: 1f
                        if (fraction < current) {
                            recoveryMap[mg] = fraction
                        }
                    }
                }
            }
